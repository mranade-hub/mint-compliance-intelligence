    ...
    if has_signatures:
        ...
        for phase, info in results.get("phases", {}).items():
            for d in info.get("documents", []):
                ...
                # FORCE a fresh page for EVERY document so the header and image are never separated
                pdf.add_page() 
                ...
                # Multi_cell handles text wrapping if the document name is incredibly long
                ...
                for img_idx, sig_path in enumerate(valid_paths):
                    ...
                    try:
                        # Width scaled back to 160mm so FPDF doesn't trigger an automatic page break 
                        # trying to fit an oversized height aspect ratio
