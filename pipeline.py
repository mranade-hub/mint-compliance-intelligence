...
def get_qualities(doc_name, compliance_map):
    """Smart fetcher that matches the dynamic categories generated by the new auditor.py"""
    norm_doc = normalize_str(doc_name)
    if norm_doc in compliance_map:
        return compliance_map[norm_doc]
            
    # Substring fallback for robustness if AI slightly alters the title
...
def run_pipeline(company, project_category, project_type, log_callback=None, progress_callback=None, forced_phase=None, folder_name="Unknown"):
    ...
    try:
        ...
        for phase in working_phases:
                ...
                if not qualities:
                    if is_optional:
                        phase_results.append({
                            ...
                            "is_bypassed": True # Crucial flag to prevent artificial phase advancement & math inflation
                        })
            ...
            # FIX: Mathematical average logic. Exclude bypassed Optional files entirely from math!
        ...
        # --- REBUILT PHASE DETECTION ---
        if forced_phase and forced_phase in PHASE_ORDER:
            detected_phase = forced_phase
        else:
            detected_phase = working_phases[0]
            for phase in working_phases:
                # Requires a REAL document to score 70+ (ignores bypassed optional docs)
